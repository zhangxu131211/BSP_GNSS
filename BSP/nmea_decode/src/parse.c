...
/**
 * \brief 打印 nmeaGPGSA 结构体内容
 * @param pack 指向 nmeaGPGSA 结构体的指针
 */
void nmea_GPGSA_print(nmeaGPGSA *pack)
{
		int i;
	  int sat_count = 0;
    if (pack == NULL) {
        printf("GPGSA Packet: NULL pointer.\n");
        return;
    }

    printf("=== GPGSA Packet Dump ===\n");
    printf("Fix Type: %d\n", pack->fix_type);
    printf("PDOP: %.3f\n", pack->PDOP);
    printf("HDOP: %.3f\n", pack->HDOP);
    printf("VDOP: %.3f\n", pack->VDOP);
    
    printf("Satellites Used (PRN): ");
    for (i = 0; i < NMEA_MAXSAT; ++i) {
        if (pack->sat_prn[i] != 0) {
            printf("%d ", pack->sat_prn[i]);
            sat_count++;
        }
    }
    printf("\nTotal Satellites in Solution: %d\n", sat_count);
    printf("==========================\n");
}
/**
