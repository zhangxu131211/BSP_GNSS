...
void GPS_USART_Config(void)
{
    ...
    /* 配置串口1时钟源*/
		...
		RCC_PeriphClkInit.Usart2ClockSelection = RCC_USART2CLKSOURCE_PCLK1;
  ...
  void GPS_ProcessDMAIRQ(void)
{
	...
	if(__HAL_DMA_GET_FLAG(&DMA_Handle,DMA_FLAG_HT6) != RESET)        /* DMA 半传输完成 */
  { 
    GPS_HalfTransferEnd = 1;                //设置半传输完成标志位
		//LED1_ON;
  }
  else if(__HAL_DMA_GET_FLAG(&DMA_Handle,DMA_FLAG_TC6)!= RESET)     /* DMA 传输完成 */
  {
    // LED1_OFF;
