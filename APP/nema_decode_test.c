...
int nmea_decode_test(void)
{
    ...
    while(1)
    {
			DEBUG_PRINT("GPS_HalfTransferEnd = %d, GPS_TransferEnd = %d", GPS_HalfTransferEnd, GPS_TransferEnd);
      if(GPS_HalfTransferEnd)     /* 接收到GPS_RBUFF_SIZE一半的数据 */
      {
//    printf("Raw GPS Data (Half): ");

//    for(i = 0; i < HALF_GPS_RBUFF_SIZE; i++) {
//        printf("%c", gps_rbuff[i]); // 以字符形式输出
//    }
//    printf("\r\n");
      ...
      {
//    printf("Raw GPS Data (other_Half): ");
//    for(a = 0; a < HALF_GPS_RBUFF_SIZE; a++) {
//        printf("%c", gps_rbuff[a]); // 以字符形式输出
//    }
//    printf("\r\n");
